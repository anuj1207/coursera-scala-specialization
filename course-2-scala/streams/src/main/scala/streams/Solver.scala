...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    b.legalNeighbors.foldLeft(LazyList.empty[(Block, List[Move])]) {
      case (lList, (nB: Block, nM: Move)) => (nB, nM :: history) #:: lList
    }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    inner(initial, initial, explored)
  }

  def inner(input: LazyList[(Block, List[Move])], lazyList: LazyList[(Block, List[Move])],
            explored: Set[Block]): LazyList[(Block, List[Move])] = {
    val newNeighbors = input.flatMap{
      case (block, moves) => newNeighborsOnly(neighborsWithHistory(block, moves), explored)
    }

    if(newNeighbors.isEmpty) lazyList
    else inner(newNeighbors, lazyList ++ newNeighbors, explored ++ newNeighbors.map(_._1))
  }

  /**
   * The lazy list of all paths that begin at the starting block.
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(LazyList((startBlock, List.empty[Move])), Set(startBlock))

  /**
   * Returns a lazy list of all possible pairs of the goal block along
   * with the history how it was reached.
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter(a => done(a._1))
